...
| Token refresh | Not implemented — token is valid for a fixed `JWT_EXPIRY_MINUTES` (60 min), then the user must log in again | Poor UX for long sessions; no way to extend a session without re-entering credentials | Add a `/refresh` endpoint that issues a new token if the current one is still within a grace period, without requiring password re-entry |
...
| Default/fallback secrets | `Config.JWT_SECRET` falls back to `"dev_secret_change_me"` if the env var isn't set | If `.env` is ever missing in a deployed environment, the app would silently run with a well-known, guessable secret | Consider failing fast (raise at startup) if `JWT_SECRET` isn't explicitly set, rather than silently falling back, for any non-local environment |
