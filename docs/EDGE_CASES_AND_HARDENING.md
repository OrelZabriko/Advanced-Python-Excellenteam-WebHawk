...
| Internal errors | **Fixed** — `services/shared/error_handlers.py` is registered by all four services, so every error (including 404/405 and unhandled exceptions) returns JSON. The exception text is logged server-side only and never sent to the client | Remaining: `debug=True` is still set in each `app.py`'s `__main__` block, which only affects native runs, not the gunicorn-based Docker images | Ensure `debug=False` in any non-local environment |
| Database connection failures | **Fixed** — `psycopg2.OperationalError` is caught by the global handler and returned as a generic JSON 500. Verified that the DB host, port, and driver name do not appear in the client response | None currently | No action needed |
